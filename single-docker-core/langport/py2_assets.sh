...
function py2_compile()
{
    ...
    echo "Wow, it is Python. We do not need to compile it!" &> result/compile.out
    returner=`echo $?`
...
function py2_execut()
{
	...
	docker run -v "${pathdocker}":/c0r3/Job --name py2_execut1 py2_execut
	docker rm -f py2_execut1
