...
function pas_compile()
{
	...
	docker run -v "${pathdocker}":/c0r3/Job --name pas_compile1 pas_compile
	docker rm -f pas_compile1
...
function pas_execut()
{
	...
	docker run -v "${pathdocker}":/c0r3/Job --name pas_execut1 pas_execut
	docker rm -f pas_execut1
