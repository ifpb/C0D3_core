...
function java_compile()
{
	...
	docker run -v "${pathdocker}":/c0r3/Job --name java_compile1 java_compile
	docker rm -f java_compile1
...
function java_execut()
{
	...
	docker run -v "${pathdocker}":/c0r3/Job --name java_execut1 java_execut
	docker rm -f java_execut1
